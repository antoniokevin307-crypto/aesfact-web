...
async function readData(){
  ...
  try {
    ...
    // Read gallery
    const { data: galleryData } = await supabase.from('gallery').select('image');
    data.gallery = (galleryData || []).map(g => g.image).filter(Boolean);

    // Read aesfact
    const { data: aesfactData } = await supabase.from('aesfact').select('*').eq('id', 'aesfact').limit(1);
    if(aesfactData && aesfactData.length > 0){
      data.aesfact = {
